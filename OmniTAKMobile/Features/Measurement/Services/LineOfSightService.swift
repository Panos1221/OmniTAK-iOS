...
class LineOfSightService: NSObject, ObservableObject {
    ...
    /// (canonical implementation in MeasurementCalculator)
    func calculateDistance(from start: CLLocationCoordinate2D,
                           to end: CLLocationCoordinate2D) -> Double {
        return MeasurementCalculator.haversineDistance(from: start, to: end)
    }

    /// Calculate bearing from start to end point (canonical formula in
